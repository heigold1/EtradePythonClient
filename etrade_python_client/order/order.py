...
class Order:
    ...
    def place_order(self, order_string: str):

        try:
            pattern = r"(\w+)\s+BUY\s+([\d,]+)\s+\$(\d+\.\d+)"
            match = re.match(pattern, order_string)
            if match:

                symbol = match.group(1)
                order_action = "BUY"
                quantity = int(match.group(2).replace(',', ''))
                limit_price = float(match.group(3))
                ...
                payload = payload.format(client_order_id, price_type, order_term,
                                         limit_price, symbol, order_action,
                                         quantity)
                ...
                if response is not None and response.status_code == 200:
                    parsed = json.loads(response.text)
                    logger.debug("Brent - Preview Response Body: %s", json.dumps(parsed, indent=4, sort_keys=True))
                    data = response.json()
                    print("\nPreview Order:")
                    ...
                    if data is not None and "PreviewOrderResponse" in data and "PreviewIds" in data[
                        ...
                        for previewids in data["PreviewOrderResponse"]["PreviewIds"]:
                            print("Preview ID: " + str(previewids["previewId"]))
                            preview_ids.append(previewids["previewId"])

                        preview_ids_section = "<PreviewIds>\n"
                        for preview_id in preview_ids:
                            preview_ids_section += f"    <previewId>{preview_id}</previewId>\n"
                        preview_ids_section += "</PreviewIds>"
                        ...
                        payload = payload.format(client_order_id, price_type, order_term,
                                                 limit_price, symbol, order_action,
                                                 quantity, preview_ids_section)

                        # Make API call for POST request
                        response = self.session.post(url, header_auth=True, headers=headers, data=payload)
                        logger.debug("Brent Order Response Request Header: %s", response.request.headers)
                        logger.debug("Brent Order Response Request payload: %s", payload)
                ...
                messagebox.showerror("Error", "Clipboard text is in the wrong format.")
